let table1 = document.getElementById('table1').onclick = function (e) {
  if (e.target.tagName != 'th') return;
  let th = e.target;
  sortTable(th.cellIndex, th.dataset.type, table1);
};

let table2 = document.getElementById('table2').onclick = function (e) {
  if (e.target.tagName != 'th') return;
  let th = e.target;
  ...
  if (goods.length) {
    ...
    for (let i = 0; i < goods.length; i++) {
      ...
      <tr class="align-middle">
        ...
        <td>${goods[i][3]}</td>
        <td><button class="good_delete btn-danger" data-delete="${goods[i][0]}">Удалить</button></td>
        <td><button class="good_delete btn-primary" data-goods="${goods[i][0]}">В корзину</button></td>
      </tr>
      `)
      if (goods[i][4] > 0) {
